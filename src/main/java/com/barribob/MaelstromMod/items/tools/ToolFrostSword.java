...
public class ToolFrostSword extends ToolSword implements ISweepAttackOverride
{
	...
	for(int i = 0; i < 5; i++)
	{
	    ParticleManager.spawnEffect(player.world, new Vec3d(target.posX, target.posY + 1, target.posZ).add(new Vec3d(ModRandom.getFloat(targetEntitySize), ModRandom.getFloat(targetEntitySize), ModRandom.getFloat(targetEntitySize))), new Vec3d(0.4, 0.4, 0.7f));
	}
    }
