...
/**
 * 
 * Generates a large tree with plum leaves and wood
 *
 */
public class WorldGenBigPlumTree extends WorldGenAbstractTree
{
	...
	this.foliageCoords = Lists.<WorldGenBigPlumTree.FoliageCoordinates>newArrayList();
	this.foliageCoords.add(new WorldGenBigPlumTree.FoliageCoordinates(this.basePos.up(k), j));
	...
	for (; k >= 0; --k)
	{
		...
		for (int l = 0; l < i; ++l)
		{
			...
			{
			    this.foliageCoords.add(new WorldGenBigPlumTree.FoliageCoordinates(blockpos, blockpos2.getY()));
			}
		    }
		}
	    }
	...
	for (int j = -i; j <= i; ++j)
	{
		...
		if (Math.pow((double) Math.abs(j) + 0.5D, 2.0D) + Math.pow((double) Math.abs(k) + 0.5D, 2.0D) <= (double) (p_181631_2_ * p_181631_2_))
		{
			...
			{
			    this.setBlockAndNotifyAdequately(this.world, blockpos, plum);
			}
			else
			{
	...
	{
	    this.crosSection(pos.up(i), this.leafSize(i), leaf, generatePlumLeaves);
	}
    }
	...
	Block block = ModBlocks.PLUM_LOG;
	...
	for (WorldGenBigPlumTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
	{
	    ...
	    {
		this.limb(blockpos, worldgenbigtree$foliagecoordinates, ModBlocks.PLUM_LOG);
	    }
	}
    }
